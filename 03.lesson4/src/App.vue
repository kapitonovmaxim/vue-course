<template>
  <div class="main">
    <header class="header">
      ...
      <sale-popup v-if="showPopup" @close-popup="onClosePopup()"></sale-popup>
    </header>
    <h1>Каталог товаров <button class="add_product">Добавить товар</button></h1>
    <div class="catalog-search">
      <search-input v-model="searchQuery"/>
    </div>
    <div class="products">
      <product-card v-for="product in filteredProducts" :key="product.id" :product="product" :in_cart="isProductInBasket(product.id)" @click-card-buy-button="handleCardBuyButton"/>
    </div>
