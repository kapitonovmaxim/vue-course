<template>
    <div class="product">
        ...
        <div class="product__price">{{ formatPriceToRub(product.price) }}</div>

        <div v-if="getCartProduct(product.id)" class="product-quantity-controls">
            <button @click.stop="decreaseQuantity()" class="quantity-btn">-</button>
            <span class="quantity-value">{{ getProductAmount }}</span>
...
const { add, remove, getCartProduct, updateAmount } = store

const getProductAmount = computed(() => getCartProduct(props.product.id)?.amount ?? 0)
...
const addToCart = (quantity = 1) => add(props.product, quantity)

const decreaseQuantity = () => {
    if (getProductAmount.value > 1) {
        updateAmount(props.product.id, getProductAmount.value - 1)
    } else {
